...
#ruledef bw8cpu {
    ...
    ; 8-bit Stores
    store a, [{abs: u16}]           => opcode(OP_STORE_A_ABS)       @ abs
    store a, [{dp: u8}]             => opcode(OP_STORE_A_DP)        @ dp
    store a, [x, #{idx: s8}]        => opcode(OP_STORE_A_X_IDX)     @ idx
    store a, [x, a]                 => opcode(OP_STORE_A_X_A)
    store a, [x, b]                 => opcode(OP_STORE_A_X_B)
    store a, [x, c]                 => opcode(OP_STORE_A_X_C)
    store a, [x, d]                 => opcode(OP_STORE_A_X_D)
    store a, [y, #{idx: s8}]        => opcode(OP_STORE_A_Y_IDX)     @ idx
    store a, [y, a]                 => opcode(OP_STORE_A_Y_A)
    store a, [y, b]                 => opcode(OP_STORE_A_Y_B)
    store a, [y, c]                 => opcode(OP_STORE_A_Y_C)
    store a, [y, d]                 => opcode(OP_STORE_A_Y_D)
    store a, [sp, #{idx: u8}]       => opcode(OP_STORE_A_SP_IDX)    @ idx
    store a, [sp, a]                => opcode(OP_STORE_A_SP_A)
    store a, [sp, b]                => opcode(OP_STORE_A_SP_B)
    store a, [sp, c]                => opcode(OP_STORE_A_SP_C)
    store a, [sp, d]                => opcode(OP_STORE_A_SP_D)

    store a, [x]                    => asm{store a, [x, #0]}
    store a, [y]                    => asm{store a, [y, #0]}

    store b, [{abs: u16}]           => opcode(OP_STORE_B_ABS)       @ abs
    store b, [{dp: u8}]             => opcode(OP_STORE_B_DP)        @ dp
    store b, [x, #{idx: s8}]        => opcode(OP_STORE_B_X_IDX)     @ idx
    store b, [x, a]                 => opcode(OP_STORE_B_X_A)
    store b, [x, b]                 => opcode(OP_STORE_B_X_B)
    store b, [x, c]                 => opcode(OP_STORE_B_X_C)
    store b, [x, d]                 => opcode(OP_STORE_B_X_D)
    store b, [y, #{idx: s8}]        => opcode(OP_STORE_B_Y_IDX)     @ idx
    store b, [y, a]                 => opcode(OP_STORE_B_Y_A)
    store b, [y, b]                 => opcode(OP_STORE_B_Y_B)
    store b, [y, c]                 => opcode(OP_STORE_B_Y_C)
    store b, [y, d]                 => opcode(OP_STORE_B_Y_D)
    store b, [sp, #{idx: s8}]       => opcode(OP_STORE_B_SP_IDX)    @ idx
    store b, [sp, a]                => opcode(OP_STORE_B_SP_A)
    store b, [sp, b]                => opcode(OP_STORE_B_SP_B)
    store b, [sp, c]                => opcode(OP_STORE_B_SP_C)
    store b, [sp, d]                => opcode(OP_STORE_B_SP_D)

    store b, [x]                    => asm{store b, [x, #0]}
    store b, [y]                    => asm{store b, [y, #0]}

    store c, [{abs: u16}]           => opcode(OP_STORE_C_ABS)       @ abs
    store c, [{dp: u8}]             => opcode(OP_STORE_C_DP)        @ dp
    store c, [x, #{idx: s8}]        => opcode(OP_STORE_C_X_IDX)     @ idx
    store c, [x, a]                 => opcode(OP_STORE_C_X_A)
    store c, [x, b]                 => opcode(OP_STORE_C_X_B)
    store c, [x, c]                 => opcode(OP_STORE_C_X_C)
    store c, [x, d]                 => opcode(OP_STORE_C_X_D)
    store c, [y, #{idx: s8}]        => opcode(OP_STORE_C_Y_IDX)     @ idx
    store c, [y, a]                 => opcode(OP_STORE_C_Y_A)
    store c, [y, b]                 => opcode(OP_STORE_C_Y_B)
    store c, [y, c]                 => opcode(OP_STORE_C_Y_C)
    store c, [y, d]                 => opcode(OP_STORE_C_Y_D)
    store c, [sp, #{idx: s8}]       => opcode(OP_STORE_C_SP_IDX)    @ idx
    store c, [sp, a]                => opcode(OP_STORE_C_SP_A)
    store c, [sp, b]                => opcode(OP_STORE_C_SP_B)
    store c, [sp, c]                => opcode(OP_STORE_C_SP_C)
    store c, [sp, d]                => opcode(OP_STORE_C_SP_D)

    store c, [x]                    => asm{store c, [x, #0]}
    store c, [y]                    => asm{store c, [y, #0]}

    store d, [{abs: u16}]           => opcode(OP_STORE_D_ABS)       @ abs
    store d, [{dp: u8}]             => opcode(OP_STORE_D_DP)        @ dp
    store d, [x, #{idx: s8}]        => opcode(OP_STORE_D_X_IDX)     @ idx
    store d, [x, a]                 => opcode(OP_STORE_D_X_A)
    store d, [x, b]                 => opcode(OP_STORE_D_X_B)
    store d, [x, c]                 => opcode(OP_STORE_D_X_C)
    store d, [x, d]                 => opcode(OP_STORE_D_X_D)
    store d, [y, #{idx: s8}]        => opcode(OP_STORE_D_Y_IDX)     @ idx
    store d, [y, a]                 => opcode(OP_STORE_D_Y_A)
    store d, [y, b]                 => opcode(OP_STORE_D_Y_B)
    store d, [y, c]                 => opcode(OP_STORE_D_Y_C)
    store d, [y, d]                 => opcode(OP_STORE_D_Y_D)
    store d, [sp, #{idx: s8}]       => opcode(OP_STORE_D_SP_IDX)    @ idx
    store d, [sp, a]                => opcode(OP_STORE_D_SP_A)
    store d, [sp, b]                => opcode(OP_STORE_D_SP_B)
    store d, [sp, c]                => opcode(OP_STORE_D_SP_C)
    store d, [sp, d]                => opcode(OP_STORE_D_SP_D)
    ...
    ; 16-bit Stores
    store x, [{abs: u16}]           => opcode(OP_STORE_X_ABS)       @ abs
    store x, [{dp: u8}]             => opcode(OP_STORE_X_DP)        @ dp
    store x, [x, #{idx: s8}]        => opcode(OP_STORE_X_X_IDX)     @ idx
    store x, [y, #{idx: s8}]        => opcode(OP_STORE_X_Y_IDX)     @ idx
    store x, [sp, #{idx: u8}]       => opcode(OP_STORE_X_SP_IDX)    @ idx

    store x, [x]                    => asm{store x, [x, #0]}
    store x, [y]                    => asm{store x, [y, #0]}

    store y, [{abs: u16}]           => opcode(OP_STORE_Y_ABS)       @ abs
    store y, [{dp: u8}]             => opcode(OP_STORE_Y_DP)        @ dp
    store y, [x, #{idx: s8}]        => opcode(OP_STORE_Y_X_IDX)     @ idx
    store y, [y, #{idx: s8}]        => opcode(OP_STORE_Y_Y_IDX)     @ idx
    store y, [sp, #{imm: u8}]       => opcode(OP_STORE_Y_SP_IDX)    @ idx

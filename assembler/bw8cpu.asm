...
#ruledef bw8cpu {
    ...
    je x                        => opcode(OP_JE_X)
    je y                        => opcode(OP_JE_Y)
    jz {abs: u16}               => opcode(OP_JE_ABS)           @ abs
    jz {rel: bw8cpu_reladdr}    => opcode(OP_JE_REL)           @ rel
    jz x                        => opcode(OP_JE_X)
    jz y                        => opcode(OP_JE_Y)
    ...
    jnz {abs: u16}              => opcode(OP_JNE_ABS)          @ abs
    jnz {rel: bw8cpu_reladdr}   => opcode(OP_JNE_REL)          @ rel
    jnz x                       => opcode(OP_JNE_X)
    jnz y                       => opcode(OP_JNE_Y)

    jc {abs: u16}               => opcode(OP_JC_ABS)           @ abs
    jc {rel: bw8cpu_reladdr}    => opcode(OP_JC_REL)           @ rel
    jc x                        => opcode(OP_JC_X)
    jc y                        => opcode(OP_JC_Y)
    jb {abs: u16}               => opcode(OP_JC_ABS)           @ abs
    jb {rel: bw8cpu_reladdr}    => opcode(OP_JC_REL)           @ rel
    jb x                        => opcode(OP_JC_X)
    jb y                        => opcode(OP_JC_Y)
    jnae {abs: u16}             => opcode(OP_JC_ABS)           @ abs
    jnae {rel: bw8cpu_reladdr}  => opcode(OP_JC_REL)           @ rel
    jnae x                      => opcode(OP_JC_X)
    jnae y                      => opcode(OP_JC_Y)

    jnc {abs: u16}              => opcode(OP_JNC_ABS)          @ abs
    jnc {rel: bw8cpu_reladdr}   => opcode(OP_JNC_REL)          @ rel
    jnc x                       => opcode(OP_JNC_X)
    jnc y                       => opcode(OP_JNC_Y)
    jnb {abs: u16}              => opcode(OP_JNC_ABS)          @ abs
    jnb {rel: bw8cpu_reladdr}   => opcode(OP_JNC_REL)          @ rel
    jnb x                       => opcode(OP_JNC_X)
    jnb y                       => opcode(OP_JNC_Y)
    jae {abs: u16}              => opcode(OP_JNC_ABS)          @ abs
    jae {rel: bw8cpu_reladdr}   => opcode(OP_JNC_REL)          @ rel
    jae x                       => opcode(OP_JNC_X)
    jae y                       => opcode(OP_JNC_Y)

    jbe {abs: u16}              => opcode(OP_JBE_ABS)          @ abs
    jbe {rel: bw8cpu_reladdr}   => opcode(OP_JBE_REL)          @ rel
    jbe x                       => opcode(OP_JBE_X)
    jbe y                       => opcode(OP_JBE_Y)
    jna {abs: u16}              => opcode(OP_JBE_ABS)          @ abs
    jna {rel: bw8cpu_reladdr}   => opcode(OP_JBE_REL)          @ rel
    jna x                       => opcode(OP_JBE_X)
    jna y                       => opcode(OP_JBE_Y)

    ja {abs: u16}               => opcode(OP_JA_ABS)           @ abs
    ja {rel: bw8cpu_reladdr}    => opcode(OP_JA_REL)           @ rel
    ja x                        => opcode(OP_JA_X)
    ja y                        => opcode(OP_JA_Y)
    jnbe {abs: u16}             => opcode(OP_JA_ABS)           @ abs
    jnbe {rel: bw8cpu_reladdr}  => opcode(OP_JA_REL)           @ rel
    jnbe x                      => opcode(OP_JA_X)
    jnbe y                      => opcode(OP_JA_Y)

    jl {abs: u16}               => opcode(OP_JL_ABS)           @ abs
    jl {rel: bw8cpu_reladdr}    => opcode(OP_JL_REL)           @ rel
    jl x                        => opcode(OP_JL_X)
    jl y                        => opcode(OP_JL_Y)
    jnge {abs: u16}             => opcode(OP_JA_ABS)           @ abs
    jnge {rel: bw8cpu_reladdr}  => opcode(OP_JA_REL)           @ rel
    jnge x                      => opcode(OP_JA_X)
    jnge y                      => opcode(OP_JA_Y)

    jge {abs: u16}              => opcode(OP_JGE_ABS)          @ abs
    jge {rel: bw8cpu_reladdr}   => opcode(OP_JGE_REL)          @ rel
    jge x                       => opcode(OP_JGE_X)
    jge y                       => opcode(OP_JGE_Y)
    jnl {abs: u16}              => opcode(OP_JGE_ABS)          @ abs
    jnl {rel: bw8cpu_reladdr}   => opcode(OP_JGE_REL)          @ rel
    jnl x                       => opcode(OP_JGE_X)
    jnl y                       => opcode(OP_JGE_Y)

    jle {abs: u16}              => opcode(OP_JLE_ABS)          @ abs
    jle {rel: bw8cpu_reladdr}   => opcode(OP_JLE_REL)          @ rel
    jle x                       => opcode(OP_JLE_X)
    jle y                       => opcode(OP_JLE_Y)
    jng {abs: u16}              => opcode(OP_JGE_ABS)          @ abs
    jng {rel: bw8cpu_reladdr}   => opcode(OP_JGE_REL)          @ rel
    jng x                       => opcode(OP_JGE_X)
    jng y                       => opcode(OP_JGE_Y)

    jg {abs: u16}               => opcode(OP_JG_ABS)           @ abs
    jg {rel: bw8cpu_reladdr}    => opcode(OP_JG_REL)           @ rel
    jg x                        => opcode(OP_JG_X)
    jg y                        => opcode(OP_JG_Y)
    jnle {abs: u16}             => opcode(OP_JG_ABS)           @ abs
    jnle {rel: bw8cpu_reladdr}  => opcode(OP_JG_REL)           @ rel
    jnle x                      => opcode(OP_JG_X)
    jnle y                      => opcode(OP_JG_Y)
}

...
namespace BW8cpu {
	...
	void falling(BW8cpu* cpu) {
		...
		switch (lines.dbus_load) {
			...
			case DbusLoad::MEM:
				...
				cpu->write(compute_addr(cpu->BR, cpu->addr_out), cpu->dbus, mem_io, cpu->supervisor_mode, data_code);
	...
	void dbus_assert(BW8cpu* cpu) {
		...
		switch (lines.dbus_assert) {
			...
			case DbusAssert::MEM:
				...
				cpu->dbus = cpu->read(compute_addr(cpu->BR, cpu->addr_out), mem_io, cpu->supervisor_mode, data_code);
				break;
			case DbusAssert::MSB:
				cpu->dbus = cpu->xfer >> 8;
				break;
	...
	uint32_t compute_addr(uint8_t bank, uint16_t vaddr) {
		(((bank & 0xF) << 0xF) | vaddr) & 0xFFFFF;
	}
}